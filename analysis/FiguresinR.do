...
// This script will do the wollowing:
...
******************************************
** Prepare and export data for Figure 2 **
******************************************
...
// Loop over age groups, years, and methods and store results in a 1 row x 2 column matrix that contains
		...
		forvalues method = 2/4 { // not including ster
			...
			if r(N) == 0 {
				...
				if r(N) == 0 {
					...
					svy, subpop(if exclude == 0 & cpmethodcat4 == `method' & agegroup == `agegroup' & period == `period'): mean p 
					// Store preg rate and variange in a matrix
					matrix pregs`agegroup'`period'`method' = e(b), e(V)
				}
			}
		}
		// Calculate overall pregnancy rate for each age group and period
		svy, subpop(if exclude == 0 & agegroup == `agegroup' & period == `period'): mean p 
		matrix pregs`agegroup'`period' = e(b), e(V)
	}
}
...
putexcel set "$NSFGKeep/pregratesbymethod", replace
putexcel A1 = ("age") B1 = ("period") C1 = ("method") D1 = ("pregrate") E1 = ("var_pregrate")
...
forvalues agegroup = 1/2 {
	forvalues period = 1/3 {
		...
		matrix pregs = `agegroup', `period', `method' , pregs`agegroup'`period'`method'
	...
	matrix pregs = `agegroup', `period', 0, pregs`agegroup'`period'
	putexcel A`row' = matrix(pregs)
	local row = `row' + 1
	}
}
